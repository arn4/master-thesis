...
from .ode._config import scalar_type
...
class PhaseDiagramInitialConditions():
  ...
  ACHTUNG: we have to discuss if setting student as linear combination of teacher can have
  negative influence on the effect we would like to study!
  """
  ...
  def weights(self, d):
    Wteacher = np.sqrt(d)*orth((normalize(self.rng.randn(self.k, d), axis=1, norm='l2')).T).T
    W0 = self.M @ Wteacher
    return Wteacher, W0
...
class RandomNormalInitialConditions():
  ...
  def __init__(self, p, k, d, sigma=1., seed = None, spherical = False):
    self.p = p
    self.k = k
    self.d = d

    rng = np.random.default_rng(seed)
    self.Wteacher = rng.normal(size=(k, d))
    self.W0 = rng.normal(size=(p, d), scale = sigma)
    if spherical:
      "Force the weights vectors to be on the sphere of radius sqrt(d)"
      self.Wteacher = np.sqrt(d) * normalize(self.Wteacher, axis = 1)
      self.W0 = np.sqrt(d) * normalize(self.W0, axis = 1)

    self.P = self.Wteacher @ self.Wteacher.T / scalar_type(d)
    self.M = self.W0 @ self.Wteacher.T / scalar_type(d)

# This file is just a less mess version of first_plateau_exit_point.py.
# I've removed all the experiments, leaving only what is really needed.
# It's the file I will use for thesis plots
...
@np.vectorize
def numerical_exit_point(k,p,d,gamma,noise, exit_threshold):
  T = exit_threshold
...
def theoretical_exit_point(k,p,d,gamma,noise,exit_threshold):
  T = exit_threshold
...
def preprocess_simulation_data(k,p,d,gamma,noise,verbose=True):
  try:
    with open(f'computation-database/new-extracted-data/npy/first-plateau-exit-k={k}-p={p}-d={d}-gamma={gamma}-noise={noise}.npy', 'rb') as f:
      times = np.load(f)
    ...
    if verbose:
        print(f'Computing: k = {k}, p = {p}, d = {d}, gamma = {gamma}, noise = {noise}', flush = True)
    sample_risks = []
    macros_risks = []
    ode_risks = []
    ...
    with open(f'computation-database/new-extracted-data/npy/first-plateau-exit-k={k}-p={p}-d={d}-gamma={gamma}-noise={noise}.npy', 'wb') as f:
      np.save(f, times)
  ...
  # except ValueError:
  #   print(f'Pickle error: k = {k}, p = {p}, d = {d}, gamma = {gamma}, noise = {noise}', flush = True)

  return times, sample_risks, macros_risks, ode_risks, ode_times

def compute_exit_points(k,p,d,gamma,noise, exit_threshold, figure=True, save_folder='computation-database/new-extracted-data/'):
  ...
  num_exit_point = numerical_exit_point(k,p,d,gamma, noise, exit_threshold)
  theo_exit_point = theoretical_exit_point(k,p,d,gamma, noise, exit_threshold)
  ...
  if figure:
    with plot_style():
      ...
      ax.set_xlim(1e0,1e2)
      ax.set_yscale('log')
      ax.axhline((1.-exit_threshold)*initial_risk(k,p,d), lw=1.5, ls='--',c='orange', label = r'$T$ line')
      ax.axhline(initial_risk(k,p,d), lw=.7, ls='-.',c='black', label = r'$\mathcal{R}_\text{apprx}{(0)}$')

      ax.plot(ode_times, ode_risks.T,ls='-',lw=1.,marker='',ms=5.,c='yellow',alpha=0.00125*np.sqrt(d))
      ax.plot(times, macros_risks.T,ls='-',lw=1.,marker='',ms=5.,c='red',alpha=0.00125*np.sqrt(d))
      ax.plot(times, np.mean(macros_risks, axis=0), label=f'Average traj.',ls='-',lw=1.,marker='',ms=2.,c='b')
      ax.plot(times, risk_at(times, k,p,d,gamma,noise), label = r'$\mathcal{R}_\text{apprx}$')
      ax.plot(ode_times, np.mean(ode_risks, axis = 0), label = 'Average ODE')

      # ax.axvline(macros_exit_from_mean, lw=.8, ls='--',c='green', label = f'Exit point of mean = {macros_exit_from_mean}')

      ax.axhline(gamma*noise/(1-6*gamma), lw=.7, ls='-.',c='blue', label = r'$\mathcal{R}_\text{apprx}{(+\infty)}$')
      ax.legend()
      fig.savefig(save_folder+f'first-plateau-exit-k={k}-p={p}-d={d}-gamma={gamma}-noise={noise}.pdf', format = 'pdf', bbox_inches = 'tight')

      ax.set_xlim(1e-2,1e1)
      ax.set_ylim(initial_risk(k,p,d)*0.995,initial_risk(k,p,d)*1.0003)
      ax.axvline(num_exit_point, lw=1.8, ls='--',c='orange', label = f'$t^\\text{{(gpr,num)}}_e = \\num{{{num_exit_point:.3}}}$')
      ax.axvline(theo_exit_point, lw=1.8, ls='--',c='green', label = f'$t^\\text{{(gpr,theo)}}_e = \\num{{{theo_exit_point:.3}}}$')
      ax.axvline(mean_of_macros_exits, lw=1.8, ls='--',c='red', label = f'$t_e = \\num{{{mean_of_macros_exits:.3}}}$')
      ax.legend()
      fig.savefig(save_folder+f'first-plateau-exit-k={k}-p={p}-d={d}-gamma={gamma}-noise={noise}-zoom.pdf', format = 'pdf', bbox_inches = 'tight')
...
def costant_alpha_plot(k, p_list, d, alpha, noise, exit_threshold):
  ...
  with plot_style():
    ...
    ax.set_ylabel('$t_e$')
    ax.set_xscale('log')
    ax.set_xlim(1.85,max(p_list)*1.15)
    ax.set_ylim(0,2)
    # ax.set_yscale('log')
    linspace = np.linspace(max(min(p_list),2,k)*0.85, max(p_list)*1.15, 1000)

    ax.plot(linspace, numerical_exit_point(k, linspace, d, alpha*linspace,noise,exit_threshold), label = 'Theoretical Exit Point')
    
    ax.plot(p_list, mean_exit_macros, marker='v', ls = '', label = 'Mean of Traj. Exit Points')
...
def d_plot(k, p, d_list, alpha, noise, exit_threshold):
  mean_exit_macros, mean_exit_sample, exit_mean_macro, exit_mean_sample = zip(*[compute_exit_points(k,p,d,alpha*p,noise,exit_threshold,False) for d in d_list])
  with plot_style():
    fig, ax = plt.subplots(figsize=(5.5,5.5))
    ax.set_xlabel('$d$')
    ax.set_ylabel('$t_e$')
    ax.set_xscale('log')
    ax.set_xlim(min(d_list)*0.85,max(d_list)*1.15)
    # ax.set_ylim(0,2)
    # ax.set_yscale('log')
    linspace = np.linspace(min(d_list)*0.85, max(d_list)*1.15, 1000)

    # ax.plot(linspace, numerical_exit_point(k, p, linspace, alpha*p,noise,exit_threshold), label = 'Numerical Exit Point')
    ax.plot(linspace, theoretical_exit_point(k, p, linspace, alpha*p,noise,exit_threshold), label = 'Theoretical Exit Point')
    
    ax.plot(d_list, mean_exit_macros, marker='v', ls = '', label = 'Mean of Macros Exit Points')
    # ax.plot(d_list, exit_mean_macro, marker='v', ls = '', label = 'Exit of Macro Mean')
    ax.legend()

    fig.savefig('tmp/test_fig_d.pdf')
...
if __name__ == '__main__':
  ...
  alpha = .1
  noise = 1e-3
  save_folder='computation-database/new-extracted-data/alpha0.1/'
  ...
  # alpha = .2
  # noise = 1e-3
  # save_folder='computation-database/new-extracted-data/alpha0.2/'

  
  exit_threshold = 0.0005

  param_list = [(k,p,d, alpha*p, noise, exit_threshold, True,save_folder) for k,p,d in list(product(k_list,p_list,d_list))]
  ...
  # d_plot(k=1,p=1,d_list=[100,1000,2500,5000,7500,10000,12500,15000],alpha=0.1,noise=noise,exit_threshold=exit_threshold)
